...
from units import Knight, Archer
...
class Grid:
        ...
        for row in self.tiles:
            for tile in row:
                ...
                if self.selected_unit:
                    ...
                    elif self.__is_within_reach(tile) and not tile.unit:
                        self.in_reach.append(tile)
                        tile.color = "#83a598"
                        tile.border_color = "#458588"

                    elif self.selected_tile and self.selected_tile.unit:
    ...
    def __attack_unit(self, unit, target):
        ...
        print(f"{unit.name} attacking {target.name}")
    ...
    def add_unit(self, unit, row, col):
        match unit.lower():
            ...
            case "archer":
                unit_to_add = Archer(row, col, self)
